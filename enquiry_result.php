...
Enter Train No: <select id="tno" name="tno" required><br><br>
<?php

require "db.php";
            
            $cdquery="SELECT DISTINCT trainno FROM classseats";
...
<br><br>
Enter Class: <select id="class" name="class" required>
<?php

require "db.php";
            
            $cdquery="SELECT cname FROM class";
            $cdresult=mysqli_query($conn,$cdquery);
...
No. of Seats: <select  name="nos" required>
<option >1</option>
  <option >2</option>
  <option >3</option>
  <option>4</option>
